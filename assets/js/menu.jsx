...
export default function run_menu(root) {
  ReactDOM.render(<JoinMenu />, root);
}
...
class JoinMenu extends React.Component {
  ...
  handleSubmit(event) {
    if (this.state.value != '') {
      window.location='game/' + this.state.value;
    }
  }
  ...
  render() {
    return (
      <div>
        <h2 id="index-page">Join a Game</h2>
    ...
    return (
      <div className="game">
        <div className="row">
          ...
          <div className="col-sm-6 d-flex justify-content-center">
            <button className="btn btn-primary" onClick={() => this.restart()}>{buttonMsg}</button>
            <button className="btn btn-primary" onClick={() => console.log("Hello.")}>Send Message</button>
          </div>
        </div>
